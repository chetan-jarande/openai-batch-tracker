...
def create_mock_data(count: int = 15) -> List[MockBatch]:
    ...
    for i in range(1, count + 1):
        ...
        elif status in [BatchStatus.IN_PROGRESS, BatchStatus.FINALIZING, BatchStatus.VALIDATING]:
             # *** FIX START ***
             # Calculate completed count first
        ...
        mock_batches.append(
            MockBatch(
                ...
                created_at=datetime.fromtimestamp(created_unix - random.randint(0, 300), tz=timezone.utc) # System time slightly after OpenAI time
            )
        )
    return mock_batches
...
async def read_dummy_dashboard(request: Request):
    ...
    try:
        ...
        return HTMLResponse(content=f"<h1>Error</h1><p>Could not render dummy dashboard: {e}</p>", status_code=500)
